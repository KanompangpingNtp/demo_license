...
class AdminHealthHazardApplicationController extends Controller
{
    ...
    //                 // เลขหนังสือ
    //                 $number = HealthLicenseNumberLogs::where('type', $detail['confirm_option'])->orderBy('id', 'desc')->first();
    //                 $run_book = $number ? $number->book + 1 : 1;
    //                 $run_number = $number ? $number->number + 1 : 1;

    //                 $numberLog = new HealthLicenseNumberLogs();
    //                 $numberLog->health_license_id = $input['id'];
    ...
    //                 if ($numberLog->save()) {
    //                     return redirect()->route('HealthHazardApplicationAdminPayment')->with('success', 'บันทึกเรียบร้อยแล้ว');
    //                 }
    //             }
    //         }
    //     }
