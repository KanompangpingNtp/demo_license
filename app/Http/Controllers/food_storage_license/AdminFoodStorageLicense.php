...
class AdminFoodStorageLicense extends Controller
{
    ...
    // public function FoodStorageLicenseExpire()
    // {
    //     $ninetyDaysFromNow = Carbon::now()->addDays(90);

    //     // ดึงเฉพาะข้อมูลที่ status 10 หรือ 11 และมีวันหมดอายุน้อยกว่า 90 วันจากวันนี้
    //     $forms = FoodStorageInformations::whereHas('details', function ($query) {
    //         $query->whereIn('status', [10, 11]);
    //     })
    //         ->with(['user', 'details', 'files', 'replies'])
    //         ->get()
    //         ->filter(function ($form) use ($ninetyDaysFromNow) {
    ...
    //                 return Carbon::parse($latestPayment->expiration_date)->lessThanOrEqualTo($ninetyDaysFromNow);
    //             }

    //             return false;
    //         });

    //     return view('admin.food_storage_license.expire-details', compact('forms'));
    // }
